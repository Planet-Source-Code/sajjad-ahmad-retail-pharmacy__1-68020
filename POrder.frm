...
Begin VB.Form frmPOrder 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin prjChameleon.chameleonButton cmdsave 
         ...
         MCOL            =   16777215
         MPTR            =   99
         MICON           =   "POrder.frx":0000
         PICN            =   "POrder.frx":031A
         UMCOL           =   -1  'True
         SOFT            =   0   'False
         PICPOS          =   0
         NGREY           =   0   'False
      ...
      Begin prjChameleon.chameleonButton cmdundo 
         ...
         MCOL            =   16777215
         MPTR            =   99
         MICON           =   "POrder.frx":0850
         PICN            =   "POrder.frx":0B6A
         UMCOL           =   -1  'True
         SOFT            =   0   'False
         PICPOS          =   0
         NGREY           =   0   'False
      ...
      Begin prjChameleon.chameleonButton cmdclose 
         ...
         MCOL            =   16777215
         MPTR            =   99
         MICON           =   "POrder.frx":1006
         PICN            =   "POrder.frx":1320
         UMCOL           =   -1  'True
         SOFT            =   0   'False
         PICPOS          =   0
         NGREY           =   0   'False
      ...
      Begin prjChameleon.chameleonButton cmdnewpro 
         ...
         MCOL            =   16777215
         MPTR            =   99
         MICON           =   "POrder.frx":17DD
         PICN            =   "POrder.frx":1AF7
         UMCOL           =   -1  'True
         SOFT            =   0   'False
         PICPOS          =   0
         NGREY           =   0   'False
...
If Text1(2).Text = "" Then
    ...
    Text1(2).SetFocus
    Exit Sub
ElseIf iGrid1.CellValue(1, 1) = "" Or Val(iGrid1.CellValue(1, 10)) = 0 Then
...
ElseIf MsgBox("Are you sure to save invoice?", vbQuestion + vbYesNo) = vbYes Then
    ...
    rsPO.Open "Porders", cn, adOpenKeyset, adLockOptimistic
    With rsPO
        .AddNew
        .Fields(0) = Text1(0)
        .Fields(1) = Text1(1)
    ...
    rsPOD.Open "PorderDetails", cn, adOpenKeyset, adLockOptimistic
    With rsPOD
    ...
    'Update Product Table Packing,TP,RP,Unit_Stock
    With iGrid1
    For i = 1 To .RowCount - 1
        cn.Execute "Update Products Set Packing=" & .CellValue(i, 4) & _
        ", Pack_TP=" & .CellValue(i, 7) & ", Pack_RP=" & .CellValue(i, 7) + (.CellValue(i, 7) * 0.15) & _
        ", Unit_TP=" & .CellValue(i, 7) / .CellValue(i, 4) & ", Unit_RP=" & .CellValue(i, 7) + (.CellValue(i, 7) * 0.15) / (.CellValue(i, 4)) & _
        ", Pack_stock=pack_stock+ " & .CellValue(i, 5) + .CellValue(i, 6) & ", Unit_stock=unit_stock+ " & .CellValue(i, 5) + .CellValue(i, 6) / .CellValue(i, 4) & _
        " Where Code=" & .CellValue(i, 1)
        
        If IsDate(iGrid1.CellValue(i, 9)) = True Then
        cn.Execute "Update Products Set Expirydate=#" & iGrid1.CellValue(i, 9) & "#" & " Where Code=" & iGrid1.CellValue(i, 1)
        End If
    Next
    End With
...
Set rsPOD = Nothing
    ...
    If MsgBox("Records Updated Successfully...Do you want to Print?", vbInformation + vbYesNo) = vbYes Then
        ...
        rsINV.Open "Select * from PInvoiceqry where BillNo=" & Me.Text1(0), cn, adOpenStatic, adLockOptimistic
...
With iGrid1
    lRow = .CurRow
    .CellValue(lRow, 10) = Format(.CellValue(lRow, 5) * .CellValue(lRow, 7) - (.CellValue(lRow, 5) * .CellValue(lRow, 7) * (.CellValue(lRow, 8) / 100)), "0.00")
End With
CalculateTotals
End Sub
...
ElseIf lCol = 9 Then
    If Not IsDate(vNewValue) Then
        MsgBox "Invalid Date Entry!", vbInformation
        eResult = igEditResProceed
    End If
End If
End Sub
...
Call ColAlign
'Lock Entry Fields
If lCol = 1 Or lCol = 2 Or lCol = 3 Or lCol = 10 Then
    bCancel = True
ElseIf lCol = 5 Then
...
With iGrid1
    ...
    .AddCol "Expiry", "Expiry", igTextCenter, , 80
    .AddCol "Line Total", "Line Total", igTextRight, , 100
    .ColHeaderBackColor(9) = vbRed
    .RowCount = 1
    .Header.Flat = True
...
Text1(3) = 0
For i = 1 To iGrid1.RowCount
    Text1(3) = Format(Val(Text1(3)) + Val(iGrid1.CellValue(i, 10)), "0.00")
Next
End Sub

Private Sub NewMode()
rs.Open "Select Billno from POrders", cn, adOpenKeyset, adLockOptimistic
